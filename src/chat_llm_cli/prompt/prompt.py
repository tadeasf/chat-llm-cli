...
def start_prompt(
    ...
    while True:
        ...
        console.print(Text("Chat-LLM-CLI", style="#89dceb"))  # Catppuccin Sky
        console.print(
        ...
        prompt_text = f"<tokens>[Tokens: {prompt_tokens + completion_tokens}]</tokens> "
        if config.get("budget_enabled") and config.get("budget_user"):
            prompt_text += f"<cost>[Cost: ${current_cost:.6f}]</cost>"
        prompt_color = "#cba6f7" if config["multiline"] else "ansiwhite"  # Catppuccin Mauve for multiline, white for single-line
        prompt_text += f"\n<prompt>>>></prompt>"
        ...
        style = Style.from_dict({
            'tokens': '#89b4fa',  # Catppuccin Blue
            'cost': '#f38ba8',  # Catppuccin Red
            'prompt': prompt_color,
        })
...
def save_and_open_session(config: Dict[str, Any], messages: List[Dict[str, str]]) -> None:
    ...
    """
    from chat_llm_cli.config.config import get_session_filename, SAVE_FOLDER
    from chat_llm_cli.prompt.history import save_history

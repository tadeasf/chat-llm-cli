...
BASE = Path(xdg_config_home(), "chat-llm-cli")
...
def get_session_filename() -> str:
    ...
    try:
        ...
        # If the folder still doesn't exist for some reason, start with index 1
        session_index = 1
...
def initialize_budget_manager(config: Dict[str, Any]) -> BudgetManager:
    ...
    budget_manager = BudgetManager(project_name="chat-llm-cli")
